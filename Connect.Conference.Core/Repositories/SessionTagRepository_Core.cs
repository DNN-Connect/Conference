...
namespace Connect.Conference.Core.Repositories
{
    ...
    public partial class SessionTagRepository : ServiceLocator<ISessionTagRepository, SessionTagRepository>, ISessionTagRepository
 {
        ...
        public void DeleteSessionTag(int sessionId, int tagId)
        {
             Requires.NotNull(sessionId);
            using (var context = DataContext.Instance())
            {
                context.Execute(System.Data.CommandType.Text,
                    "DELETE FROM {databaseOwner}{objectQualifier}Connect_Conference_SessionTags WHERE SessionId=@0 AND TagId=@1",
                    sessionId,tagId);
            }
        }
    ...
    public partial interface ISessionTagRepository
    {
        ...
        void DeleteSessionTag(int sessionId, int tagId);
