export default class SchedulerGrid extends React.Component {
  render() {
    var vertLines = [];
    ...
    for (var i = 0; i < this.props.locations.length; i++) {
      horLabels.push(
        <text
          x={6 + i * 100 + this.props.leftMargin}
          y="20"
          className="gridLabel"
        >
    ...
    for (var i = 0; i < this.props.height; i = i + 60) {
      horLines.push(
        <line
          ...
          y1={i}
          x2={this.props.width + this.props.leftMargin}
          y2={i}
          className="gridline"
        />
      );
      ...
      if (slot.SlotType == 0) {
        ...
        slotBands.push(
          <rect
            ...
            y={slot.StartMinutes - this.props.start}
        ...
        slotBands.push(
          <foreignObject
            x={
              this.props.locationList[slot.LocationId] * 100 +
      ...
      if (slot.SlotType == 0) {
        for (var j = 0; j < this.props.locations.length; j++) {
          var refId =
            ...
            "x" +
            this.props.locations[j].LocationId.toString();
          slots.push(
            <rect
              x={j * 100 + this.props.leftMargin}
              y={slot.StartMinutes - this.props.start}
              height={slot.DurationMins}
              width="100"
              ...
              ref={refId}
              data-slotid={slot.SlotId}
              data-locationid={this.props.locations[j].LocationId}
              id={refId}
              data-day={this.props.day}
            />
          );
